...
| `inject-prompt-forge.sh` | UserPromptSubmit — **opt-in** | Reminds the model to apply the prompting protocol (`prompting.md`): sufficiency gate → ask if info is missing; else classify, scope the context, decompose, define verifiable criteria. Trivial prompts proceed directly. **Not wired by default — see [Opt-in](#opt-in-inject-prompt-forgesh) below.** |
...
## Opt-in: `inject-prompt-forge.sh`

Unlike the other hooks, this one is **not wired by default** — neither in the plugin's
`hooks/hooks.json` nor in `config/settings.example.json`. It fires on EVERY user prompt to inject the
prompting-protocol reminder, which is intrusive for a general-purpose pack. It is the always-on twin
of the `write-prompt` skill (the on-demand version of the same capability). Enable it only if you want
the protocol nudged every turn:

- **Plugin:** add this entry to `hooks/hooks.json` under `UserPromptSubmit`:
  ```json
  { "type": "command", "command": "bash \"${CLAUDE_PLUGIN_ROOT}/hooks/inject-prompt-forge.sh\"", "timeout": 3 }
  ```
- **Manual (`settings.json`):** add the same entry under `UserPromptSubmit`, using
  `bash "$CLAUDE_PROJECT_DIR/.claude/hooks/inject-prompt-forge.sh"`.
